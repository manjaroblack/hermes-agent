...
KANBAN_GUIDANCE = (
    ...
    "detection. If a worker was already blocked but later produced a PR, a late `kanban_complete`/`kanban_block` "
    "must carry a `review_evidence` object with provider, repository, branch, immutable 40-character `head_sha`, "
    "`pr_url`, and `pr_number`; live open/non-draft exact-head green checks are required before recovery. Missing, "
    "draft, closed, diverged, red, pending, or unknown evidence stays blocked.\n"
