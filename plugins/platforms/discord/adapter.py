...
class DiscordAdapter(DiscordMediaMixin, BasePlatformAdapter):
    ...
    def _discord_auto_thread_free_response(self) -> bool:
        ...
        including an explicit ``False``. The adapter's scoped env fallback is
        used only when config is absent. The default remains ``False`` so
        existing free-response channels stay inline.
        """
        ...
        if configured == "":
            configured = self._gate_env("DISCORD_FREE_RESPONSE_AUTO_THREAD")
    ...
    def _discord_free_response_auto_thread(self) -> bool:
        """Compatibility name for the target's free-response auto-thread setting."""
        return self._discord_auto_thread_free_response()
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if auto_thread_free_response_cfg is not None:
        ...
        _env_default("DISCORD_AUTO_THREAD_FREE_RESPONSE", str(auto_thread_free_response_cfg).lower())
