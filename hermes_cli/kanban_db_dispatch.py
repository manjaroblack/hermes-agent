...
class DispatchResult:
    ...
    """Blocked review handoffs durably verified and routed this tick."""
    review_recovery_blocked: list[tuple[str, str]] = field(default_factory=list)
    """Blocked review handoffs that remain blocked with a diagnostic."""
...
def check_respawn_guard(
    ...
    # 4. Structured review evidence from a blocked worker is also an active PR
    #    signal after manual unblock, even when the worker never wrote a prose
    #    comment containing the URL.
    try:
        blocked_rows = conn.execute(
            "SELECT payload, created_at FROM task_events WHERE task_id = ? "
            "AND kind IN ('blocked', 'block_loop_detected') ORDER BY id DESC",
            (task_id,),
        ).fetchall()
    except Exception:
        blocked_rows = []
    evidence_created_at = None
    for event in blocked_rows:
        try:
            payload = json.loads(event["payload"]) if event["payload"] else {}
        except (TypeError, json.JSONDecodeError):
            payload = {}
        if isinstance(payload, dict) and isinstance(payload.get("review_evidence"), dict):
            evidence_created_at = int(event["created_at"] or 0)
            break
    if evidence_created_at is not None and evidence_created_at >= pr_cutoff:
        latest_rework = conn.execute(
            "SELECT MAX(created_at) AS created_at FROM task_events WHERE task_id = ? "
            "AND kind = 'changes_requested'", (task_id,),
        ).fetchone()
        if not latest_rework or int(latest_rework["created_at"] or 0) < evidence_created_at:
            return "active_pr"

    # A PR opened for a task that has since received reviewer changes is a
    # deliberate rework lane, not duplicate work.
    latest_rework = conn.execute(
        "SELECT MAX(created_at) AS created_at FROM task_events WHERE task_id = ? "
        "AND kind = 'changes_requested'", (task_id,),
    ).fetchone()
    latest_rework_at = int(latest_rework["created_at"] or 0) if latest_rework else 0
    latest_pr_comment = conn.execute(
        "SELECT MAX(created_at) AS created_at FROM task_comments WHERE task_id = ? "
        "AND created_at >= ? AND body GLOB '*https://github.com/*/pull/*'", (task_id, pr_cutoff),
    ).fetchone()
    if latest_rework_at and latest_pr_comment and latest_rework_at >= int(latest_pr_comment["created_at"] or 0):
        return None
...
def _dispatch_once_locked(
    ...
    *,
    ...
    result = DispatchResult()
    if not dry_run:
        recovered, blocked = _kb.recover_blocked_review_tasks(
            conn, provider=review_provider, board=board,
        )
        result.review_recovered.extend(recovered)
        result.review_recovery_blocked.extend(blocked)
    _run_reclaim_phase(
