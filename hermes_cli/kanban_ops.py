    ...
    if getattr(args, "json", False):
        _print_json({
            **{k: getattr(res, k)
            ...
            "rate_limited": getattr(res, "rate_limited", []),
            "respawn_guarded": getattr(res, "respawn_guarded", []),
            "skipped_locked": getattr(res, "skipped_locked", False),
            "memory_pressure": getattr(res, "memory_pressure", None),
            "review_recovered": getattr(res, "review_recovered", []),
            "review_recovery_blocked": getattr(res, "review_recovery_blocked", []),
        }, ascii=True)
        return 0
