...
def decompose_triage_task(
    ...
    with write_txn(conn):
        root_row = conn.execute(
            "SELECT id, status, tenant, workspace_kind, workspace_path, project_id, legacy_unscoped "
...
def _insert_decomposed_child(
    ...
    """
    from hermes_cli.kanban_db import (
        _new_task_id, _canonical_assignee, _append_event, _board_meta_for,
        _project_snapshot, _project_branch_name,
    )
    ...
    body = child.get("body")
    branch_name = None
    if root_row["project_id"] and child_ws_kind == "worktree":
        snapshot = _project_snapshot(_board_meta_for(None))
        if snapshot is not None:
            from types import SimpleNamespace
            project = SimpleNamespace(
                id=snapshot["project_id"], slug=snapshot["project_slug"],
                name=snapshot["project_name"], primary_path=snapshot["project_primary_path"],
            )
            branch_name = _project_branch_name(project, new_id, child.get("title"))
    conn.execute(
        "INSERT INTO tasks "
        "(id, title, body, assignee, status, workspace_kind, "
        " workspace_path, branch_name, project_id, legacy_unscoped, tenant, created_at, created_by) "
        "VALUES (?, ?, ?, ?, 'todo', ?, ?, ?, ?, ?, ?, ?, ?)",
        (
            ...
            branch_name, root_row["project_id"], int(root_row["legacy_unscoped"] or 0),
