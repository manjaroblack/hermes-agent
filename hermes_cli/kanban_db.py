...
BOARD_PROJECT_FIELDS = (
    "project_id", "project_slug", "project_name", "project_primary_path",
)
_REVIEW_CHILD_RE = re.compile(r"(?:review|code[\s_-]*review|sdlc[\s_-]*review)", re.IGNORECASE)
_REVIEW_CHILD_ASSIGNEES = frozenset({"hermes-review", "reviewer", "code-reviewer"})
...
def _invoke_kanban_hook(event: str, **fields: Any) -> None:
    """Invoke direct test/embedded registrations without triggering plugin discovery.

    The lifecycle facade lazily discovers plugins; that is correct for normal
    delivery but would replace callbacks registered by an already-running
    embedded manager between the hot-path gate and fire. Directly registered
    callbacks are therefore a stable fast path.
    """
    try:
        from hermes_cli import plugins
        manager = plugins.get_plugin_manager()
        callbacks = tuple(getattr(manager, "_hooks", {}).get(event, ()))
    except Exception:
        callbacks = ()
    if callbacks:
    ...
    try:
        ...
        invoke_hook(event, **fields)
    except Exception as exc:  # pragma: no cover - observer isolation
        _log.debug("kanban lifecycle hook %s failed: %s", event, exc)


def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    """Best-effort lifecycle hook after the write transaction commits."""
    _invoke_kanban_hook(event, task_id=task_id, profile_name=_hook_profile_name(), **fields)
...
def _kanban_observer_consumed(event: str) -> bool:
    """Hot-path short-circuit that also sees direct manager registrations."""
    try:
        from hermes_cli import plugins
        manager = plugins.get_plugin_manager()
        if getattr(manager, "_hooks", {}).get(event):
            return True
    except Exception:
        pass
    try:
...
def _fire_dispatch_tick_hook(
    ...
    try:
        ...
        _invoke_kanban_hook(
            ...
            dry_run=dry_run, outcome=outcome, result=result,
        )
    except Exception as exc:  # pragma: no cover - defensive
...
def _project_snapshot(meta: Mapping[str, Any]) -> Optional[dict[str, str]]:
    ...
    if not primary.is_absolute():
        raise BoardProjectError("INVALID_PRIMARY_PATH", "project_primary_path must be absolute")
    return values
...
def write_board_metadata(
    ...
    *,
    ...
    """Create/update ``board.json`` while preserving unmentioned fields."""
    _assert_not_delegated_child_mutation()
    slug = _normalize_board_slug(board) or DEFAULT_BOARD
    ...
    *,
    ...
    if project_id:
        try:
            ...
            if exc.code != "UNKNOWN_PROJECT" or any(
                value is not None for value in (project_slug, project_name, project_primary_path)
            ):
                raise
            # An imported/exported board may carry an opaque project id whose
            # local project catalog is unavailable. Keep the id but discard
            # exporter-local paths rather than refusing to create the shell.
            snapshot = None
        if snapshot is None:
            ...
            meta = write_board_metadata(normed, name=name, description=description, icon=icon, color=color)
        else:
            meta = write_board_metadata(
                normed,
                name=name,
...
def _audit_issue(
    issues: list[dict[str, Any]], code: str, message: str, *,
    board: Optional[str] = None, task_id: Optional[str] = None, severity: str = "error",
) -> None:
...
def audit_boards() -> dict[str, Any]:
    """Run a read-only integrity audit over board/project scope."""
    ...
    if root.is_dir():
        for child in sorted(root.iterdir(), key=lambda p: p.name.lower()):
            ...
            try:
                ...
                continue
            if not slug or slug == DEFAULT_BOARD or not _dir_holds_board(child):
                continue
    ...
    for slug, raw, metadata_error in raw_entries:
        ...
        if raw is not None:
            meta.update(raw)
        meta["slug"] = slug
        board = {"slug": slug, "metadata_present": bool(raw is not None), "project_id": meta.get("project_id")}
        boards.append(board)
        if metadata_error:
            _audit_issue(issues, "MALFORMED_BOARD_METADATA", metadata_error, board=slug)
            continue
        if slug != DEFAULT_BOARD and raw is None:
            _audit_issue(issues, "MISSING_BOARD_METADATA", "named board has no board.json", board=slug)
        pid = str(meta.get("project_id") or "").strip()
        if pid:
            ...
            if snapshot is not None:
                ...
                if not primary.is_absolute() or not primary.is_dir():
                    _audit_issue(
                        issues, "INVALID_PRIMARY_PATH",
                        f"project primary path is not an existing absolute directory: {primary}", board=slug,
                    )
                if str(meta.get("default_workdir") or "").strip() != str(primary.resolve()):
                    _audit_issue(
                        issues, "PROJECT_WORKDIR_MISMATCH",
                        "default_workdir does not match project_primary_path", board=slug,
                    )
        else:
            _audit_issue(
                issues, "UNSCOPED_LEGACY",
                "board has no project snapshot; use a project board for new durable work",
                board=slug, severity="warning",
            )
        for row in _read_only_task_rows(kanban_db_path(slug)):
            ...
            if not task_project:
                if not bool(row.get("legacy_unscoped", 0)):
                    _audit_issue(
                        issues, "UNSCOPED_TASK_ON_SCOPED_BOARD",
                        "task has no project_id on a project-scoped board", board=slug, task_id=str(row.get("id")),
                    )
            elif task_project != pid:
                _audit_issue(
                    issues, "TASK_PROJECT_MISMATCH",
                    f"task project {task_project!r} differs from board project {pid!r}",
                    board=slug, task_id=str(row.get("id")),
                )
    for pid, slugs in project_to_boards.items():
        if len(slugs) > 1:
            for slug in slugs:
                _audit_issue(
                    issues, "DUPLICATE_PROJECT_BINDING",
                    f"project {pid!r} is present on multiple boards: {', '.join(sorted(slugs))}", board=slug,
                )
    ...
    if current_path.exists():
        ...
        if not current or not any(b["slug"] == current for b in boards):
            _audit_issue(issues, "STALE_CURRENT_BOARD", f"current board pointer {current!r} does not resolve to a board")
    invalid_codes = {
        "MALFORMED_BOARD_METADATA", "MISSING_BOARD_METADATA", "MISSING_PROJECT_SNAPSHOT",
        "INVALID_PRIMARY_PATH", "PROJECT_WORKDIR_MISMATCH",
    }
    conflict_codes = {
        "DUPLICATE_PROJECT_BINDING", "TASK_PROJECT_MISMATCH", "UNSCOPED_TASK_ON_SCOPED_BOARD",
    }
    for board in boards:
        board_issues = [issue for issue in issues if issue.get("board") == board["slug"]]
        board["issues"] = board_issues
        board["status"] = (
            "invalid" if any(issue["code"] in invalid_codes for issue in board_issues)
            else "conflict" if any(issue["code"] in conflict_codes for issue in board_issues)
            else "scoped" if board.get("project_id") else "legacy_unscoped"
        )
...
def _validate_board_notify_connection(conn: sqlite3.Connection, board: str) -> None:
    current_path = _connection_db_path(conn)
    if current_path is None:
        return
    expected_path = kanban_db_path(board=board).resolve()
    if current_path != expected_path:
        raise ValueError(f"board {board!r} database does not match the live connection")


def get_board_notify(
    conn: sqlite3.Connection, board: Optional[str] = None, *, platform: str = "discord",
) -> Optional[dict[str, str]]:
    """Return the validated board notification pin, if any."""
    if str(platform or "").strip().lower() != "discord" or board is None:
        return None
...
def list_board_notify(conn: sqlite3.Connection, board: Optional[str] = None) -> list[dict[str, str]]:
...
def _board_notify_db_path(board: str) -> Path:
    slug = _normalize_board_slug(board) or DEFAULT_BOARD
    return kanban_home() / "kanban.db" if slug == DEFAULT_BOARD else board_dir(slug) / "kanban.db"
...
def _board_notify_conflict(
    conn: sqlite3.Connection, *, destination: Mapping[str, object], board: str,
) -> Optional[str]:
    from hermes_cli import kanban_board_notify as board_notify
    ...
    for slug, path in _iter_board_notify_db_paths():
        ...
        if current_path is not None and path.resolve() == current_path:
            continue
        try:
            with _read_only_connection(path) as other:
...
def set_board_notify(
    ...
    user_id_alt: Optional[str] = None, chat_type: Optional[str] = None,
    notifier_profile: Optional[str] = None, delivery_mode: Optional[str] = None,
    delivery_metadata: Optional[Mapping[str, Any]] = None, board: Optional[str] = None,
    replace_existing: bool = True,
) -> dict[str, str]:
    del user_id, user_id_alt, chat_type, notifier_profile, delivery_mode, delivery_metadata
    from hermes_cli import kanban_board_notify as board_notify
    ...
    with _BOARD_NOTIFY_THREAD_LOCK:
        if not replace_existing:
            ...
            if existing is not None:
                return existing
        conflict = _board_notify_conflict(conn, destination=canonical, board=board_id)
        if conflict is not None:
            raise ValueError(f"notification destination already belongs to board {conflict!r}")
        return board_notify.set_board_notify(conn, board_id, canonical)


def _remove_notify_subs_for_task_platform(conn: sqlite3.Connection, task_id: str, platform: str) -> int:
    with write_txn(conn):
        cur = conn.execute(
            "DELETE FROM kanban_notify_subs WHERE task_id = ? AND LOWER(platform) = ?",
            (task_id, str(platform or "").strip().lower()),
        )
    return int(cur.rowcount or 0)


def remove_notify_subs_for_task_platform(conn: sqlite3.Connection, task_id: str, platform: str) -> int:
...
def purge_discord_task_subs(conn: sqlite3.Connection) -> int:
    with write_txn(conn):
        cur = conn.execute("DELETE FROM kanban_notify_subs WHERE LOWER(platform) = 'discord'")
    return int(cur.rowcount or 0)
...
def _auto_subscribe_on_create_enabled() -> bool:
    try:
        ...
        return bool(cfg_get(load_config(), "kanban", "auto_subscribe_on_create", default=True))
    except Exception:
        return True
...
def subscribe_notify_source_on_create(
    ...
    if not _auto_subscribe_on_create_enabled():
        if normalized_platform == "discord" and get_board_notify(conn, board=board) is not None:
    ...
    def subscribe_card() -> bool:
        ...
        add_notify_sub(
            conn, task_id=task_id, platform=normalized_platform, chat_id=normalized_chat_id,
            thread_id=normalized_thread_id or None, user_id=user_id, user_id_alt=user_id_alt,
    ...
    eligible = (
        board_id != DEFAULT_BOARD and not board_meta.get("legacy_unscoped")
        and bool(board_meta.get("project_id")) and bool(normalized_thread_id)
    ...
    try:
        set_board_notify(
            conn, platform="discord", chat_id=normalized_chat_id,
            thread_id=normalized_thread_id, board=board_id, replace_existing=False,
        )
    ...
    def from_row(cls, row: sqlite3.Row) -> "Task":
        ...
        return cls(
            ...
            **{col: g(col) for col in _TASK_OPTIONAL_COLUMNS},
            legacy_unscoped=bool(g("legacy_unscoped")),
...
def create_task(
    ...
    if legacy_unscoped or explicit_no_project:
        ...
        if project_id is not None and str(project_id).strip() != board_project_id:
            raise BoardProjectError(
                "TASK_PROJECT_MISMATCH",
                f"task project {str(project_id).strip()!r} differs from board project {board_project_id!r}",
            )
    ...
    if project_obj is None and requested_project_id == board_project_id and not legacy_unscoped:
        snapshot = _project_snapshot(board_meta)
        if snapshot is not None:
            from hermes_cli import projects_db as _pdb
            project_id = snapshot["project_id"]
            project_obj = _pdb.Project(
                id=snapshot["project_id"], slug=snapshot["project_slug"],
                name=snapshot["project_name"], created_at=0,
                primary_path=snapshot["project_primary_path"],
            )
            project_repo = snapshot["project_primary_path"]
            ...
            with write_txn(conn, allow_nested=True):
                ...
                conn.execute(
                    ...
                    (
                        ...
                        branch_name, project_id, 1 if legacy_unscoped else 0, tenant, idempotency_key,
                ...
                _inherit_notify_subs(conn, task_id, parents, created_at=now, board=board)
            return task_id
...
def _inherit_notify_subs(
    ...
    created_at: Optional[int] = None, board: Optional[str] = None,
) -> None:
    ...
    if resolved_board is None:
        candidate = get_current_board()
        current_path = _connection_db_path(conn)
        expected_path = kanban_db_path(board=candidate).resolve()
        if current_path is not None and current_path == expected_path:
            resolved_board = candidate
    board_pin = get_board_notify(conn, board=resolved_board)
...
def _review_metadata_has_candidate(metadata: Any) -> bool:
    if not isinstance(metadata, Mapping):
        return False
    return bool(
        any(key in metadata for key in (
            "review_evidence", "pr_url", "pull_request_url", "pr_number",
            "head_sha", "commit_sha", "head_branch",
        ))
        or isinstance(metadata.get("review"), Mapping)
        or isinstance(metadata.get("pr"), Mapping)
    )
...
def _latest_blocked_review_metadata(
    ...
    rows = conn.execute(
        ...
        "AND kind IN ('blocked', 'block_loop_detected', 'review_recovery_blocked') ORDER BY id DESC",
        (task_id,),
    ).fetchall()
    for row in rows:
        try:
    ...
    rows = conn.execute(
        "SELECT metadata FROM task_runs WHERE task_id = ? AND outcome = 'blocked' ORDER BY id DESC",
        (task_id,),
    ).fetchall()
    for row in rows:
        try:
...
def _review_child_ids(conn: sqlite3.Connection, task_id: str) -> list[str]:
    rows = conn.execute(
        "SELECT t.id, t.title, t.body, t.assignee, t.skills, t.workflow_template_id "
        "FROM tasks t JOIN task_links l ON l.child_id = t.id "
        "WHERE l.parent_id = ? ORDER BY t.created_at ASC, t.id ASC", (task_id,),
    ).fetchall()
    review_ids: list[str] = []
    for row in rows:
        try:
        ...
        graph_text = " ".join(str(row[key] or "") for key in ("title", "workflow_template_id"))
        if (
            str(row["assignee"] or "").casefold().strip() in _REVIEW_CHILD_ASSIGNEES
...
def _review_reviewer(metadata: Any, prior_request: Optional[dict]) -> str:
    ...
    if value is None:
        try:
            ...
            value = cfg.get("review_reviewer", cfg.get("blocked_review_reviewer"))
        except Exception:
            value = None
    return str(value or "hermes-review").strip() or "hermes-review"


def _review_recovery_event(
    conn: sqlite3.Connection, task_id: str, *, evidence: dict, diagnostic: str, source: str,
) -> None:
    ...
    if latest is not None and latest["kind"] == "review_recovery_blocked":
        try:
            ...
            prior = {}
        if isinstance(prior, dict) and prior.get("diagnostic") == diagnostic and prior.get("review_evidence") == evidence:
            return
    _append_event(conn, task_id, "review_recovery_blocked", {
        "diagnostic": diagnostic, "source": source, "review_evidence": evidence,
    })


def _review_recovery_already_routed(conn: sqlite3.Connection, task_id: str, evidence: dict) -> Optional[str]:
    row = conn.execute(
        "SELECT payload FROM task_events WHERE task_id = ? AND kind = 'review_recovery_routed' "
        "ORDER BY id DESC LIMIT 1", (task_id,),
    ).fetchone()
    if row is None:
        return None
    try:
        ...
        return None
    lane = payload.get("lane") if isinstance(payload, dict) and payload.get("review_evidence") == evidence else None
...
def recover_blocked_completion(
    conn: sqlite3.Connection, task_id: str, *, metadata: Optional[dict] = None,
    summary: Optional[str] = None, result: Optional[str] = None,
    expected_run_id: Optional[int] = None, provider=None, dry_run: bool = False,
) -> tuple[bool, Optional[str]]:
    """Safely route a blocked implementation after immutable PR verification."""
    from hermes_cli.kanban_review_recovery import verify_live_review_state

    task = get_task(conn, task_id)
    if task is None:
        return False, "task not found"
    candidate_metadata = metadata if _review_metadata_has_candidate(metadata) else None
    ...
    if evidence is None:
        ...
        if not dry_run:
            with write_txn(conn):
                _review_recovery_event(conn, task_id, evidence={}, diagnostic=diagnostic, source="metadata")
        return False, diagnostic
    evidence_dict = evidence.as_dict()
    already = _review_recovery_already_routed(conn, task_id, evidence_dict)
    if already:
        return True, already
    ok, diagnostic, provider_snapshot = verify_live_review_state(evidence, provider=provider)
    if not ok:
        if not dry_run:
            with write_txn(conn):
                ...
                if current is not None and current["status"] == "blocked":
                    _review_recovery_event(
                        conn, task_id, evidence=evidence_dict,
                        diagnostic=diagnostic or "live provider state is unknown", source="provider",
                    )
    ...
    if len(child_ids) > 1:
        diagnostic = "blocked completion has multiple downstream review children; remove the duplicate lane before retrying recovery"
        if not dry_run:
            with write_txn(conn):
                _review_recovery_event(conn, task_id, evidence=evidence_dict, diagnostic=diagnostic, source="graph")
        return False, diagnostic
    if child_ids and prior_request is not None:
        diagnostic = "review graph has both a downstream child and a same-card review_requested handoff; choose exactly one review lane"
        if not dry_run:
            with write_txn(conn):
                _review_recovery_event(conn, task_id, evidence=evidence_dict, diagnostic=diagnostic, source="graph")
        return False, diagnostic
    ...
    recovery_metadata = {
        "review_evidence": evidence_dict, "review_provider_state": provider_snapshot,
        "review_recovery": "blocked_completion", "review_lane": lane,
    }
    routed_run_id: Optional[int] = None
    with write_txn(conn):
        current = conn.execute("SELECT status, current_run_id, assignee FROM tasks WHERE id = ?", (task_id,)).fetchone()
        if current is None:
            return False, "task not found"
        if current["status"] != "blocked":
            existing = _review_recovery_already_routed(conn, task_id, evidence_dict)
            return (True, existing) if existing else (False, f"task is {current['status']}; blocked recovery is no longer applicable")
        locked_children = _review_child_ids(conn, task_id)
        locked_prior = _latest_review_request_payload(conn, task_id)
        if len(locked_children) > 1 or (bool(locked_children) != bool(child_ids)) or ((locked_prior is not None) != (prior_request is not None)):
            diagnostic = "review graph changed during recovery; choose exactly one stable review lane and retry"
            _review_recovery_event(conn, task_id, evidence=evidence_dict, diagnostic=diagnostic, source="graph")
            return False, diagnostic
        if expected_run_id is not None:
            latest_block = conn.execute(
                "SELECT id FROM task_runs WHERE task_id = ? AND outcome = 'blocked' ORDER BY id DESC LIMIT 1", (task_id,),
            ).fetchone()
        ...
        if not _parents_satisfied(conn, task_id):
            ...
            _review_recovery_event(conn, task_id, evidence=evidence_dict, diagnostic=diagnostic, source="dependency")
            return False, diagnostic
        if lane == "downstream":
            cur = conn.execute(
                ...
                "WHERE id = ? AND status = 'blocked'", (result, int(time.time()), task_id),
            )
            if cur.rowcount != 1:
                existing = _review_recovery_already_routed(conn, task_id, evidence_dict)
                return (True, existing) if existing else (False, "task changed during recovery")
            routed_run_id = _end_run(conn, task_id, outcome="completed", status="done", summary=summary or result, metadata=recovery_metadata)
            if routed_run_id is None:
                routed_run_id = _synthesize_ended_run(conn, task_id, outcome="completed", summary=summary or result, metadata=recovery_metadata)
            _append_event(conn, task_id, "completed", {
                "summary": _review_summary_line(summary or result, "Implementation recovered; downstream review released."),
                "review_recovery": True, "review_lane": lane, "review_evidence": evidence_dict,
            }, run_id=routed_run_id)
        else:
            reviewer = _canonical_assignee(reviewer) or "hermes-review"
            cur = conn.execute(
                "UPDATE tasks SET status = 'review', assignee = ?, claim_lock = NULL, claim_expires = NULL, "
                "worker_pid = NULL, block_kind = NULL, block_recurrences = 0 WHERE id = ? AND status = 'blocked'",
                (reviewer, task_id),
            )
            if cur.rowcount != 1:
                existing = _review_recovery_already_routed(conn, task_id, evidence_dict)
                return (True, existing) if existing else (False, "task changed during recovery")
            routed_run_id = _end_run(conn, task_id, outcome="review_requested", status="review", summary=summary, metadata=recovery_metadata)
            if routed_run_id is None:
                routed_run_id = _synthesize_ended_run(conn, task_id, outcome="review_requested", summary=summary, metadata=recovery_metadata)
            _append_event(conn, task_id, "review_requested", {
                "summary": _review_summary_line(summary, "Implementation recovered; review requested."),
                "implementer": current["assignee"], "reviewer": reviewer,
                "review_recovery": True, "review_evidence": evidence_dict,
            }, run_id=routed_run_id)
        _append_event(conn, task_id, "review_recovery_routed", {
            "lane": lane, "reviewer": reviewer if lane == "same_card" else None,
            "child_id": child_ids[0] if child_ids else None,
            "review_evidence": evidence_dict, "provider_state": provider_snapshot,
        }, run_id=routed_run_id)
    if lane == "downstream":
        _clear_failure_counter(conn, task_id)
        recompute_ready(conn)
        ...
        _fire_kanban_lifecycle_hook(
            ...
            assignee=done_task.assignee if done_task else None, run_id=routed_run_id,
...
def recover_blocked_review_tasks(
    conn: sqlite3.Connection, *, provider=None, board: Optional[str] = None,
) -> tuple[list[tuple[str, str]], list[tuple[str, str]]]:
    """Retry durable review evidence for blocked cards once per dispatch tick."""
    recovered: list[tuple[str, str]] = []
    blocked: list[tuple[str, str]] = []
    rows = conn.execute("SELECT id FROM tasks WHERE status = 'blocked' ORDER BY created_at, id").fetchall()
    for row in rows:
        task_id = row["id"]
        metadata, _source = _latest_blocked_review_metadata(conn, task_id)
        if not _review_metadata_has_candidate(metadata):
            continue
        ok, route = recover_blocked_completion(conn, task_id, metadata=metadata, provider=provider)
        if ok:
            recovered.append((task_id, str(route or "same_card")))
        elif route:
            blocked.append((task_id, str(route)))
...
def complete_task(
    ...
    if prior_status == "blocked":
        recovered, recovery_result = recover_blocked_completion(
            conn, task_id, metadata=metadata, summary=summary, result=result,
...
def block_task(
    ...
    with write_txn(conn):
        ...
        if isinstance(run_metadata, dict) and isinstance(run_metadata.get("review_evidence"), Mapping):
            payload["review_evidence"] = dict(run_metadata["review_evidence"])
        if isinstance(run_metadata, dict) and run_metadata.get("reviewer"):
            payload["reviewer"] = str(run_metadata["reviewer"])
...
def list_profiles_on_disk() -> list[str]:
    """Return configured profile names, including the implicit default."""
    return sorted(row["name"] for row in _profile_roster_metadata())


def _profile_roster_metadata() -> list[dict[str, Any]]:
    """Return the global profile roster and role descriptions."""
    try:
        ...
        try:
            ...
            if root.is_dir():
                names.append({"name": "default", "on_disk": True, "description": "", "description_auto": False, "has_description": False})
                ...
                if profiles_dir.is_dir():
                    for entry in sorted(profiles_dir.iterdir(), key=lambda p: p.name):
                        if entry.is_dir() and re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", entry.name):
                            description = ""
                            try:
                                raw = json.loads((entry / "profile.yaml").read_text(encoding="utf-8"))
                                description = str(raw.get("description") or "").strip() if isinstance(raw, dict) else ""
                            except (OSError, ValueError):
                                pass
                            names.append({"name": entry.name, "on_disk": True, "description": description, "description_auto": False, "has_description": bool(description)})
            return names
        except Exception:
            return []
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    names = sorted(on_disk | set(counts))
    return [
        {
            "name": name,
...
_PLUGIN_COMPAT_LAZY = {
    '_RESPAWN_GUARD_PR_WINDOW': ('hermes_cli.kanban_db_dispatch', '_RESPAWN_GUARD_PR_WINDOW'),
