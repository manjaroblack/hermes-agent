...
def _sandbox_tools_for_session(enabled_tools: Optional[List[str]]) -> frozenset:
    """Resolve sandbox tools without widening worker-only attachment access."""
...
# Per-tool stub specifications: (signature, docstring, args_dict_expr — the JSON payload sent over RPC).
_TOOL_SPECS = {
    ...
    "terminal": ("command: str, timeout: int = None, workdir: str = None",
        '"""Run a shell command (foreground only). Returns dict with "output": "...", "exit_code": N."""',
...
def _missing_hermes_tools_import_hint(m, enabled_tools) -> str:
    ...
    if missing.startswith("kanban_"):
        return (
            ...
            "For binary files, use terminal(command=\"hermes kanban attach TASK PATH\") "
            "to attach the local path."
        )
...
def generate_hermes_tools_module(enabled_tools: List[str],
    ...
    return header + "\n".join(
        ...
        for name, (sig, doc, args_expr) in sorted(_TOOL_SPECS.items()) if name in set(enabled_tools)
    )
...
def _sandbox_tools_for(enabled_tools: Optional[List[str]]) -> frozenset:
    """Backward-compatible alias for session-aware sandbox tool resolution."""
    return _sandbox_tools_for_session(enabled_tools)
...
_TOOL_DOC_LINES = [
    ...
    ("kanban_attach", "  kanban_attach(task_id=None, filename=None, content_base64=None, content_type=None, board=None) -> dict\n"
     "    Stores inline base64 file bytes as a real task attachment (up to 25 MB)."),
    ("kanban_attach_url", "  kanban_attach_url(task_id=None, url=None, filename=None, content_type=None, board=None) -> dict\n"
     "    Downloads an HTTP(S) URL and stores it as a real task attachment (up to 25 MB)."),
    ("kanban_attachments", "  kanban_attachments(task_id=None, board=None) -> dict\n"
     "    Lists the files attached to a task, including paths and metadata."),
]
