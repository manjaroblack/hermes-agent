...
def bridge_core_env_settings(yaml_cfg: dict, platforms_data: dict) -> None:
    ...
    # Slack's blacklist is safety-critical and must still bridge when the
    # optional Slack plugin dependencies are unavailable during config load.
    slack_cfg = yaml_cfg.get("slack", {})
    if isinstance(slack_cfg, dict) and "ignored_channels" in slack_cfg and not os.getenv("SLACK_IGNORED_CHANNELS"):
        raw = slack_cfg.get("ignored_channels")
        values = raw if isinstance(raw, (list, tuple, set)) else str(raw).split(",")
        os.environ["SLACK_IGNORED_CHANNELS"] = ",".join(
